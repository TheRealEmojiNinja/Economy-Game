...
def doElectricalOutageEvent(game_object : g.GameData, province : p.Province) -> None:
    ...
    print(random.choice(game_object.EVENT_DESCRIPTIONS["Electrical_Outage"]).format(province=province.getName(), days=days))
...
def doMineCollapseEvent(game_object : g.GameData, province : p.Province) -> None:
    ...
    if random.random() < prob:
        ...
        print(random.choice(game_object.EVENT_DESCRIPTIONS["Mine_Collapse"]).format(province=province.getName(), deaths=random.randint(5, 65)))
...
def doHeavyStormEvent(game_object : g.GameData, province : p.Province) -> None:
    ...
    if storm_cost < e.getCurrencyAmount(game_object):
        ...
        print(random.choice(game_object.EVENT_DESCRIPTIONS["Heavy_Storm_Success"]).format(cost=storm_cost, province=province.getName()))
    else:
        print(random.choice(game_object.EVENT_DESCRIPTIONS["Heavy_Storm_Failure"]).format(cost=storm_cost, province=province.getName()))
...
def doEmergencyEvent(game_object : g.GameData, province : p.Province) -> None:
    ...
    if emergency_cost < e.getCurrencyAmount(game_object):
        ...
        print(random.choice(game_object.EVENT_DESCRIPTIONS["Emergency_Success"]).format(cost=emergency_cost, province=province.getName()))
    else:
        print(random.choice(game_object.EVENT_DESCRIPTIONS["Emergency_Failure"]).format(cost=emergency_cost, province=province.getName()))
